...
export const payOrder = async (orderId, paymentResult) => {
  try {
    ...
    //lihat yg dikembalikan dari server orderRouter.put ==> res.send({ message: 'Order Update', order: updatedOrder });
    //ini sudah sukses tingak pake kartu kredit saja !
...
//list order history end poit server /api/orders/mine
export const getMyOrders = async () => {
  try {
    ...
    if (response.statusText != 'OK') {
      throw new Error(response.mineOrders.message);
    }
    //response dari server mineOrders
    return response.mineOrders;
  } catch (err) {
    return { error: err.response.mineOrders.message || err.message };
  }
};
